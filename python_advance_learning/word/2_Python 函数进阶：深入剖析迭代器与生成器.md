...
​	<font color="red">开放封闭原则：对拓展开放，对修改封闭。简单来说，就是当一个软件已经开始投入使用时，就不应该对软件的内容进行修改，这就叫做“修改封闭”。但是，如果想要给这个软件添加功能，就可以以拓展的形式添加功能，这就叫做“拓展开放”。</font>

> - <font color="blue">核心思想、执行顺序、不带参数的装饰器、带参数的装饰器。</font>
>
> 	```markdown
> 	穿衣服（使用装饰器）不会改变身体本身（不修改函数核心代码），也不会改变人们和身体互动的方式（不改变函数调用方式），但能让身体具备额外的 “属性”（比如保暖、美观，对应函数的额外功能）。
> 	```
>
> - <font color="blue">装饰器分类：</font>
>
> 	基础装饰器：（2层）外层接收被装饰函数`func`，内层对`func`进行拓展。
>
> 	进阶装饰器：（3层）外层接收传入参数，中间层接收被装饰函数`func`，内层对`func`进行拓展。
>
> - <font color="blue">如何解决元数据覆盖？</font>
>
> 	`@wraps()`装饰器放在最内层闭包之前
>
> 	```python
